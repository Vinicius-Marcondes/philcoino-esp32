...
Render and exercise the dashboard without discovery, storage, authentication, or HTTP:

```bash
EXPO_PUBLIC_PHILCOINO_DEBUG_DEVICE=1 bun run start
```

The in-memory client acknowledges UI changes, but temperature and uptime remain static. Use the device simulator for API integration work.
