...
Sharing SCK and SO while assigning an independent active-low CS to each MAX6675 is compatible with its read-only SPI interface. Firmware must keep the unselected device's CS high and never select both devices simultaneously.
