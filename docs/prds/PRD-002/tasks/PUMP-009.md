...
- This Human task remains `Todo`. Vinicius must run or directly supervise the
  target/disconnected-low-voltage matrix, classify each statement as observed,
  owner-reported, inferred, or deferred, and provide explicit acceptance before
  any acceptance checkbox or `Done` status can be recorded.
...
- No separately authorized energized evidence is recorded by this task. Pump operation, SSR output, series-switch position, flow, and physical de-energization remain unmeasured by the software.
